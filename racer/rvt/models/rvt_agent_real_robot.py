...
class RVTAgent:
    def __init__(
        ...
        self._transform_augmentation_xyz = [0.08, 0.08, 0.08] #transform_augmentation_xyz
        self._transform_augmentation_rpy = [0.0, 0.0, 45.0] #transform_augmentation_rpy
    ...
    def load_lang_model(self, use_lang_v2):
        ...
        if use_lang_v2:
            ...
            from rvt.utils.lang_enc_utils import LangModel
    ...
    def _move_pc_in_bound(self, pc, img_feat, bounds, number_pc, no_op=False):
        """
        ...
        mask = torch.zeros(size=(pc.shape[0], pc.shape[1]), 
                           dtype=torch.bool, device=pc.device)
        for i in range(pc.shape[0]):
            mask[i, number_pc[i]:] = True
    ...
    def update(
        ...
        bs = replay_sample["rot_grip_action_indicies"].shape[0]
        number_pc = replay_sample["merged_pc_number"].reshape(-1) # (b,)
        merged_pc = replay_sample["merged_pc"].squeeze(1)  # (b, n, 6)
        ...
        with torch.no_grad():
            ...
            pc, img_feat = merged_pc[:, :, :3], merged_pc[:, :, 3:]
            # img_feat = self._norm_rgb(img_feat) # TODO: check if it's RGB
            # print("action_gripper_pose\n:", action_gripper_pose)
            ...
            # print("pc shape:", number_pc)
            pc, img_feat = self._move_pc_in_bound(
                pc, img_feat, self.scene_bounds, number_pc, no_op=not self.move_pc_in_bound
            )
            # print("pc shape:", [x.shape for x in pc])
            wpt = [x[:3] for x in action_trans_con]
            # print("action_trans_con:", action_trans_con)
    ...
    def act(
        ...
        gripper_joint_positions = np.clip(np.array(observation["gripper_joint_positions"]), 0., 0.04)
        robot_state = np.array([observation["gripper_open"], *gripper_joint_positions], dtype=np.float32)  
        low_dim_state = np.array(robot_state, dtype=np.float32)
        time = (1. - (step / float(25 - 1))) * 2. - 1.
        low_dim_state = np.concatenate([low_dim_state, [time]]).astype(np.float32)
        proprio = torch.tensor(low_dim_state).float().to(self._device).unsqueeze(0)
        proprio = proprio[:, :self._net_mod.proprio_dim]
        ...
        merged_pc = torch.tensor(np.array(observation["merged_pc"], dtype=np.float32)).to(self._device)
        pc, img_feat = merged_pc[:, :3], merged_pc[:, 3:]
        # img_feat = self._norm_rgb(img_feat) # TODO: check if it's RGB
        pc = [pc]
        img_feat = [img_feat]
        ...
        if input_lang_str is not None and "drawer" in input_lang_str:
            z = continuous_action[2]
            while z < 0.79:
                z += 0.01
            continuous_action[2] = z
