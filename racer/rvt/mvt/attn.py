...
class Attention(nn.Module):  # is all you need. Living up to its name.
    ...
    def forward(self, x, context=None, mask=None):
        ...
        q = self.to_q(x) # shape [bs, seqlen, inner_dim]
        context = default(context, x)
        k, v = self.to_kv(context).chunk(2, dim=-1) # shape [bs, seqlen, inner_dim]

        q, k, v = map(lambda t: rearrange(t, "b n (h d) -> (b h) n d", h=h), (q, k, v))
        sim = einsum("b i d, b j d -> b i j", q, k) * self.scale  # shape [bs, seqlen, seqlen]
        if exists(mask):
...
class DenseBlock(nn.Module):
    def __init__(self, in_features, out_features, norm=None, activation=None, use_dropout=False):
