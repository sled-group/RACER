...
from racer.rvt.mvt.attn import (
    Conv2DBlock,
    Conv2DUpsampleBlock,
    PreNorm,
    Attention,
...
class MVT(nn.Module):
    ...
    def forward(
        ...
        proprio=None,
        lang_emb=None,
        lang_len=None, # this is true lang len, not max len
        **kwargs,
    ):
        """
        ...
        attn_mask = torch.ones(size=ins.shape[:2], dtype=torch.bool, device=ins.device)
        
        if lang_len is not None:
            for b in range(bs):
                assert lang_len[b] <= self.lang_max_seq_len, f"{lang_len[b]} > {self.lang_max_seq_len}"
                attn_mask[b, lang_len[b]: self.lang_max_seq_len] = False
        ...
        if self.self_cross_ver == 0:
            ...
            # cross attention
            for self_attn, self_ff in self.layers[len(self.layers) // 2 :]:
                x = self_attn(x, mask=attn_mask) + x
                x = self_ff(x) + x
        ...
        if self.add_lang:
            ...
            x = x[:, num_lang_tok:]
        x = self.fc_aft_attn(x)  # [B, num_img * np * np, 128]
        ...
        feat = []
        _feat = torch.max(torch.max(x, dim=-1)[0], dim=-1)[0] # [B, 128, num_img]
        _feat = _feat.view(bs, -1) # [B, 128 * num_img]
        feat.append(_feat)
        ...
        u0 = self.up0(x)   # [B * num_img, 64, h, w]
        u0 = torch.cat([u0, d0], dim=1)  # [B * num_img, 128, h, w]
        u = self.final(u0)  # [B * num_img, 64, h, w]

        # translation decoder
        trans = self.trans_decoder(u).view(bs, self.num_img, h, w)  # [B, num_img, h, w]

        hm = F.softmax(trans.detach().view(bs, self.num_img, h * w), 2).view(
            bs * self.num_img, 1, h, w
        ) # [B * num_img, 1, h, w]

        _feat = torch.sum(hm * u, dim=[2, 3])  # [B * num_img, 64]
        _feat = _feat.view(bs, -1)  # [B, num_img * 64]
        feat.append(_feat)
        feat = torch.cat(feat, dim=-1) # [B, num_img * 192]
        feat_output = self.feat_fc(feat) # [B, 72 * 3 + 2 + 2]

        out = {"trans": trans, "feat": feat_output, "raw_feat": feat}
